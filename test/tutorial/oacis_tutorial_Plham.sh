...
function setup_tutorial_Plham() {
  ...
  test `docker exec -it ${OACIS_CONTAINER_NAME} bash -c "test -f /home/oacis/tutorial/lib/plham/oacis_tutorial_Plham.sh"; echo $?` -eq 0 -a \
    ...
    `docker exec -it ${OACIS_CONTAINER_NAME} bash -c "/home/oacis/oacis/public/Result_development/work/plham/runner/CI2002.sh 1.0 0.0 1.0 12345" > /dev/null 2>&1; echo $?` -eq 0 -a \
    `docker exec -it ${OACIS_CONTAINER_NAME} bash -c "mongo --eval \"db = db.getSiblingDB('oacis_development'); db.simulators.find({\\\\\"name\\\\\":\\\\\"Plham_CI2002\\\\\"}).map (function(u) {return u.name;})[0]\"" | tail -1 | tr -d '\r'` == 'Plham_CI2002'
}
